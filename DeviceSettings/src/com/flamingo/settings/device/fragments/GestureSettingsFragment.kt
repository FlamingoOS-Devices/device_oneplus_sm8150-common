...
class GestureSettingsFragment : PreferenceFragmentCompat() {
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        hardwareManager.touchscreenGestures.forEach { gesture: TouchscreenGesture ->
            val listPreference = SystemSettingListPreference(context).apply {
            ...
            val resId = context.resources.getIdentifier(listPreference.key, "string", context.packageName)
            listPreference.title = if (resId != 0) context.getString(resId) else gesture.name
